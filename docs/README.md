...
![SlideToggleButton 外观示例](SlideToggleButton.png)
...
## 行为与事件触发规则
*   **`Click` 事件**：仅在鼠标按下点与释放点之间的距离 **小于 5 个DIP (5/96 英寸)** 时触发。距离 **大于或等于 5 个DIP** 的操作被视为滑动，不会触发 `Click`。
...
## 可能出现的问题
当前实现的**通过依赖属性来改变 `VisualState.Storyboard` 中的动画目标值**的方式可能不够稳定，可能会出现动画颜色错误的问题（仅影响视觉，**不会**导致崩溃）
